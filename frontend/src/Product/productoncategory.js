   ...
   const [updateui,setupdateui] = useState()
    const admin_menu =[
        ...
        const [activeMenus, setActiveMenus] = useState(
          ...
          function HandleOnClick(categoryName) {
            setupdateui(categoryName)
            

            // window.location.href = `/productcategory/${categoryName}`;
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const response = await axios.get(`${baseurl}/getcategoryproduct/${category_name.categoryname}`);
